...
if __name__ ==  "__main__":
    ...
    group.add_argument("-r", "--recursive", action="store_true", help="convert an entire directory of pdfs to txt")
    PARSER.add_argument("-v", "--verbose", action="store_true", help="set verbosity of cleaning")
    args = PARSER.parse_args()

    input_file = Path(args.file).resolve()
    if args.recursive:
        for name, text in pdf.get_folder_pdf_text(str(input_file)).items():
